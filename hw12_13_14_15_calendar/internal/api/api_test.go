...
type apiTestSuite struct {
	...
	client eventservice.CalendarClient
}
...
func (s *apiTestSuite) SetupSuite() {
	...
	eventservice.RegisterCalendarServer(s.srv, NewAPI(calendar.New(s.storage)))
	go func() {
...
func (s *apiTestSuite) SetupTest() {
	...
	s.NoError(err)
	s.conn = conn
	s.client = eventservice.NewCalendarClient(conn)
}
...
func (s *apiTestSuite) TestAddEvent() {
	...
	s.Run("simple case add event", func() {
		res, err := s.client.AddEvent(ctx, &eventservice.AddEventRequest{
	...
	s.Run("add unique events id", func() {
		res1, err := s.client.AddEvent(ctx, &eventservice.AddEventRequest{
			Title:     "some testing event",
			DateStart: timestamppb.New(time.Now()),
			DateEnd:   timestamppb.New(time.Now().Add(time.Hour)),
			UserId:    uuid.New().String(),
		...
		s.NoError(err)
		res2, err := s.client.AddEvent(ctx, &eventservice.AddEventRequest{
			Title:     "event_2",
			DateStart: timestamppb.New(time.Now()),
			DateEnd:   timestamppb.New(time.Now().Add(time.Hour)),
			UserId:    uuid.New().String(),
	...
	s.Run("invalid request", func() {
		_, err := s.client.AddEvent(ctx, &eventservice.AddEventRequest{Title: "some testing event"})
...
func (s *apiTestSuite) TestDeleteEvent() {
	...
	s.Run("simple case", func() {
		_, err := s.client.DeleteEvent(ctx, &eventservice.DeleteEventRequest{EventId: eventID.String()})
		s.NoError(err)
	...
	s.Run("non existent event", func() {
		_, err := s.client.DeleteEvent(ctx, &eventservice.DeleteEventRequest{EventId: uuid.New().String()})
		s.ErrorContains(err, "Event not found")
	})
}
...
func (s *apiTestSuite) TestUpdateEvent() {
	...
	eventID := s.addTestEvent()
	s.Run("simple case", func() {
		_, err := s.client.UpdateEvent(ctx, &eventservice.UpdateEventRequest{
	...
	s.Run("invalid request", func() {
		_, err := s.client.UpdateEvent(ctx, &eventservice.UpdateEventRequest{
			Event: &eventservice.Event{Title: "some testing event"},
		})
		s.ErrorContains(err, "invalid request")
	})
}
...
func (s *apiTestSuite) TestGetEventsOfDay() {
	...
	s.Run("in period day", func() {
		for _, date := range []time.Time{yesterday, today, tomorrow} {
			res, err := s.client.GetEventsOfDay(ctx, &eventservice.GetEventsRequest{Since: timestamppb.New(date)})
			s.NoError(err)
...
func (s *apiTestSuite) TestGetEventsOfWeek() {
	...
	s.Run("in period week", func() {
		for _, date := range []time.Time{time.Now(), time.Now().AddDate(0, 0, 7)} {
			res, err := s.client.GetEventsOfWeek(ctx, &eventservice.GetEventsRequest{Since: timestamppb.New(date)})
			s.NoError(err)
			s.NotEmpty(res.Events)
		}
	})
...
func (s *apiTestSuite) GetEventsOfMonth() {
	...
	s.Run("in period month", func() {
		for _, date := range []time.Time{time.Now(), time.Now().AddDate(0, 1, 0)} {
			res, err := s.client.GetEventsOfMonth(ctx, &eventservice.GetEventsRequest{Since: timestamppb.New(date)})
			s.NoError(err)
			s.NotEmpty(res.Events)
		}
	})
